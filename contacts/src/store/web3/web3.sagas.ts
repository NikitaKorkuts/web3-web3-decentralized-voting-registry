...
import {setAccounts, setCurrentAccount, setIsContractInit} from './web3.slice';
...
export function* initWeb3Saga() {
  try {
    if (window.ethereum) {
      web3 = new Web3('http://localhost:7545' || window.ethereum as string);
      yield window.ethereum.request({ method: 'eth_requestAccounts' });
      yield fork(watchOnAccountsChanged, window.ethereum);
    } else if (window.web3) {
      web3 = new Web3('http://localhost:7545' || window.web3.currentProvider as string);
    } else {
    ...
    yield put(setIsContractInit(true))
  } catch (error) {
